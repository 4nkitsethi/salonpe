...
//            });
        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {                
                Sweetalert2.fire({
                    text: `The file "${file.name}" does not match with the accepted file extensions: "${plugin.settings.extensions.join('", "')}"`,
                    icon: 'error',
                    toast: true,
        ...
        let validateMaxFiles = function (index, file) {
            ...
            if ((index + dataTransfer.items.length + plugin.settings.preloaded.length) >= plugin.settings.maxFiles) {
                ...
                Sweetalert2.fire({
                    text: `The file "${file.name}" could not be added because the limit of ${plugin.settings.maxFiles} files has been reached`,
                    icon: 'error',
                    toast: true,
                    showConfirmButton: false,
                    position: 'top-end',
